...
public class TodoMovimiento : MonoBehaviour
{
    ...
    void MoveTarget(Vector3 direction)
    {
        ...
        if (PlayerBehaviour.instance.canJump == true)
        {
            ...
            {
                LeanTween.move(levels, levels.transform.position + new Vector3(0, 0, -direction.normalized.z) * jumpDistance, timeAnimation / 2).setEase(LeanTweenType.easeInSine); //vertical abajo
            }
            if (direction.normalized.z > 0)
            {
                LeanTween.move(levels, levels.transform.position + new Vector3(0, 0, -direction.normalized.z) * jumpDistance, timeAnimation / 2).setEase(LeanTweenType.easeInSine); //vertical arriba
            }
